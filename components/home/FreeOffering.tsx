      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {items.map((item, i) => (
            <div
              key={i}
              className="bg-white/10 border border-white/15 rounded-[12px] p-5 flex items-start gap-4 hover:bg-white/15 transition-colors"
            >
              <div className="text-[#C9963A] shrink-0 mt-0.5">{icons[i]}</div>
              <p className="text-white text-sm leading-snug font-[family-name:var(--font-dm-sans)]">
                {item}
              </p>
            </div>
          ))}
