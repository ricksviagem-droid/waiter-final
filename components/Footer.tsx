      ...
      <div className="max-w-6xl mx-auto px-5 py-16">
        <div className="flex flex-col md:flex-row justify-between gap-12">
          ...
          <nav className="flex flex-col gap-3">
            {(['blog', 'exchange', 'consulting', 'planos', 'about'] as const).map((key) => (
              <Link
                key={key}
                href={`/${key}`}
                className="text-sm text-[#888] hover:text-white transition-colors font-[family-name:var(--font-dm-sans)]"
              >
                {t(`link_${key}`)}
              </Link>
            ))}
          ...
          <div className="flex gap-3">
            ...
            {/* WhatsApp */}
            <a
              href="https://wa.me/5511962794747"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="WhatsApp"
              className="w-10 h-10 rounded-full border border-[#333] flex items-center justify-center text-[#888] hover:text-white hover:border-white transition-colors"
        ...
        {/* Bottom bar */}
        <div className="mt-12 pt-8 border-t border-[#2a2a2a] flex flex-col sm:flex-row justify-between gap-2 text-xs text-[#555] font-[family-name:var(--font-dm-sans)]">
          <p>{t('copyright')}</p>
          <p>{t('proudly')}</p>
        </div>
      </div>
