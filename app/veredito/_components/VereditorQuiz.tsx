...
import type { ProfileData, IdeaData, AnalysisResult } from '@/lib/veredito/types'
...
export default function VereditorQuiz() {
  ...
  useEffect(() => {
    ...
    if (payment === 'success' && sessionId) {
      setIsVerifying(true)
      fetch(`/api/veredito/verify?session_id=${sessionId}`)
        .then((r) => r.json())
        .then(({ paid }) => {
  ...
  const runAnalysis = async (profileData: ProfileData, ideaData: IdeaData) => {
    setError(null)
    try {
      const res = await fetch('/api/veredito/analyze', {
        method: 'POST',
  ...
  const handlePay = async () => {
    setError(null)
    try {
      const res = await fetch('/api/veredito/checkout', { method: 'POST' })
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <header className="sticky top-0 z-10 bg-zinc-950/90 backdrop-blur-sm border-b border-zinc-800/50">
        <div className="max-w-xl mx-auto px-5 py-4 flex items-center justify-between">
          <button onClick={handleRestart} className="group flex items-center gap-2">
          ...
          {(step === 1 || step === 2) && (
            <ProgressBar currentStep={step as 1 | 2} />
          )}
        </div>
      </header>
