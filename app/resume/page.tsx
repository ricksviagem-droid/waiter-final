...
export default function ResumePage() {
  ...
  async function downloadPDF() {
    ...
    const line = (text: string, size: number, style: 'normal'|'bold', color: [number,number,number], indent = L) => {
    ...
    // Header
    line(resume.name || '[Your Name]', 22, 'bold', [220, 216, 255])
    y -= 2
    line(resume.title, 11, 'normal', [167, 139, 250])
    y += 2
    rule(true)

    // Summary
    section('PROFESSIONAL SUMMARY')
    line(resume.summary, 9, 'normal', [100, 90, 140])

    // Competencies
    section('CORE COMPETENCIES')
    line(resume.competencies.join('  ·  '), 9, 'normal', [140, 120, 190])

    // Experience
    if (resume.experience.length) {
      section('EXPERIENCE')
      resume.experience.forEach(exp => {
        line(`${exp.role}`, 10, 'bold', [200, 192, 240])
        y -= 2
        line(`${exp.company}  ·  ${exp.period}`, 8.5, 'normal', [120, 100, 180])
        exp.bullets.forEach(b => {
          doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(100, 90, 140)
          const wrapped = doc.splitTextToSize(`• ${b}`, W - 6)
    ...
    if (resume.education.length) {
      ...
      resume.education.forEach(e => {
        line(`${e.degree}  —  ${e.institution}  (${e.year})`, 9, 'normal', [120, 100, 180])
      })
    }

    // Languages
    section('LANGUAGES')
    line(resume.languages.join('  |  '), 9, 'normal', [140, 120, 190])

    // Certifications
    if (resume.certifications?.length) {
      section('CERTIFICATIONS')
      line(resume.certifications.join('  ·  '), 9, 'normal', [140, 120, 190])
    }

    // Additional Skills
    if (resume.additionalSkills.length) {
      section('ADDITIONAL SKILLS')
      line(resume.additionalSkills.join('  ·  '), 9, 'normal', [100, 90, 140])
    }
    ...
    doc.text('Generated by ON DUTY AI  ·  Add contact details before sending  ·  Tailor for each application', L, y)

    doc.save(`${(resume.name || 'Resume').replace(/\s+/g, '_')}_CV.pdf`)
  }
